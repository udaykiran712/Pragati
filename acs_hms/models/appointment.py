...
class Appointment(models.Model):
    ...
    READONLY_CONFIRMED_STATES = {'confirm': [('readonly', True)], 'in_consultation': [('readonly', False)], 'pause': [('readonly', True)], 'to_invoice': [('readonly', True)], 'waiting': [('readonly', True)], 'cancel': [('readonly', True)], 'done': [('readonly', True)]}
    ...
    chief_complain = fields.Char(string='Chief Complaints', states=READONLY_STATES, help="The concise statement describing the symptom, problem, condition, diagnosis, physician-recommended return, or other reason for a medical encounter.")
    present_illness = fields.Char(string='History of Present Illness', states=READONLY_STATES)

    assistant = fields.Char(string='Assistant', states=READONLY_STATES)
    provisanal_diagnosis = fields.Char(string='Provisanal Diagnosis',states=READONLY_STATES)
    final_diagnosis = fields.Char(string='Final Diagnosis',states=READONLY_STATES)
    ...
    past_history = fields.Char(string='Past History', states=READONLY_STATES, help="Past history of any diseases.")
    ...
    ref_physician_id = fields.Many2one('res.partner', ondelete='restrict', string='Referring Doctor', 
    ...
    description = fields.Text(string="Please specify the other reference")
