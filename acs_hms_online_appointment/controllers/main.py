...
class HMSWebsite(Website):
    ...
    def create_booking_data(self):
        ...
        department_ids = request.env['hr.department'].sudo().search([('company_id','=',company_id.id),('patient_department','=',True),('allowed_online_booking','=',True)])
        physician_ids = request.env['hms.physician'].sudo().search([('company_id','=',company_id.id),('allowed_online_booking','=',True)])
    ...
    def user_booking_data(self, post):
        ...
        user = request.env['res.users'].sudo().browse(request.uid)
        last_date = fields.Date.today() + timedelta(days=company_id.allowed_booking_online_days)
        ...
        values.update({
            'terms_page_link': company_id.acs_appointment_tc,
            'allow_video_call': company_id.acs_allowed_video_consultation,
    ...
    def save_appointment(self, redirect=None, **post):
        ...
        if post:
            ...
            if company_id.allowed_booking_payment:
    ...
    def appointment_payment(self, appointment, **kw):
...
class AcsCustomerPortal(CustomerPortal):
    ...
    def _get_custom_rendering_context_values(self, **kwargs):
        ...
        :rtype: dict
        """
        res = super(AcsCustomerPortal, self)._get_custom_rendering_context_values(**kwargs)
        if kwargs.get('acs_appointment_id'):
            res['acs_appointment_id'] = kwargs.get('acs_appointment_id')
        return res

    def _create_transaction(self, *args, acs_appointment_id=None, **kwargs):
        res = super()._create_transaction(*args, acs_appointment_id=acs_appointment_id, **kwargs)
        if acs_appointment_id:
            res.acs_appointment_id = acs_appointment_id
        return res
