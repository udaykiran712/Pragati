...
class PatientLabTest(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin', 'acs.hms.mixin', 'portal.mixin', 'acs.documnt.mixin', 'acs.qrcode.mixin', 'acs.documnt.view.mixin']
    ...
    test_id = fields.Many2one('acs.radiology.test', string='Test', required=True, ondelete='restrict', states=STATES, tracking=True)
    ...
    radiology_request_id = fields.Many2one('acs.radiology.request', string='Test Request', ondelete='restrict', states=STATES)
    #acsv16: cehck and remove
    #radiology_id = fields.Many2one('acs.radiology', related="radiology_request_id.radiology_id", string='Radiology', readonly=True, store=True)
